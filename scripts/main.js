...
@param : {objects} movies elements from each categories.
*/
...
const fetchMovies = async (movies) => {
    ...
    // Dynamically create images inside the carrousel of each movies categories.
    for (let i = 0; i < 7; i ++) {
...
@param : {str} ID of each movie from the click event :
Fetch call the movie ID and add it's details inside the modal.
*/
const openModal = async (id) => {
    ...
    modalElements["description"].innerHTML = `${movieDetails.long_description}
                                              ${"<br><br><strong>Actors</strong> : "}
                                              ${movieDetails.actors}`;
    ...
    modalElements["imdbScore"].innerHTML = `${movieDetails.imdb_score}
                                            ${"\n&#11088"}`;
    modalElements["directors"].innerHTML = `${"Directors : "}
    ...
    modalElements["boxOffice"].innerHTML = `${"Gross income : "} ${new Intl.NumberFormat("en-us", {style: "currency", currency: "USD"})
                                            .format(movieDetails.worldwide_gross_income)}`;
};
